...
```mermaid
flowchart LR
    UI["Widgets (many Obx)"] -->|reads many Rx| C["GetX Controller (many Rx fields)"]
...
```mermaid
flowchart LR
    UI["Page (BlocBuilder + BlocListener)"] -->|events| B["Bloc / Cubit"]
    B -->|emit| State["State (single stream)"]
    B -->|emit| Eff["Effect (one shot)"]
    State -->|builder| UI
    Eff -->|listener| UI
```
...
```mermaid
flowchart LR
    UI["Page (Obx + ever)"] -->|user intent| C["GetX Controller (cubit style)"]
    C -->|updates| St["Rx<State> (single snapshot)"]
    C -->|emits| Eff["Rxn<Effect> (one shot)"]
    St -->|Obx| UI
